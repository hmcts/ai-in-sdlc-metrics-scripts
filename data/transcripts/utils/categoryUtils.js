...
// Patterns for filtering out non-work prompts
...
function analyzePromptCategoriesForWeek(week) {
  ...
  files.forEach(filePath => {
    ...
    entries.forEach(entry => {
      ...
      // Skip system messages and CLI noise
      if (textContent.includes('<command-name>') ||
          textContent.includes('<local-command-stdout>') ||
          textContent.includes('[Request interrupted') ||
          textContent.includes('<command-message>') ||
          textContent.includes('Caveat: The messages below were generated by the user')) {
        return;
      }

      // Skip very short prompts (under 30 characters)
      if (textContent.trim().length < 30) return;

      // Skip non-work prompts (acknowledgements, meta-questions, etc.)
      const shouldFilter = FILTER_PATTERNS.some(pattern => pattern.test(textContent.trim()));
